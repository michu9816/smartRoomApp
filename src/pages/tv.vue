<template>
	<f7-page name="home">
		...
		<div class="remote" v-if="selectedTab=='remote'">
			<tvChannel v-for="(channel, index) in channels.filter(obj => obj.id == recieverStatus.currservice_serviceref)" 
				:channelData="channel" 
				:recieverIp="recieverIp"
				:key="index" 
				@sendChannelChangeSignal="sendChannelChangeSignal"></tvChannel>
			<div class="previousChannels" v-if="history.length">
				<div class="header">Ostatnie kanały</div>
				<tvHistory v-for="(item, index) in history" 
						:channelData="item" 
						:recieverIp="recieverIp"
						:key="index" 
						@sendChannelChangeSignal="sendChannelChangeSignal" ></tvHistory>
			</div>
			<div class="tvButtonRow" v-for="row in 3" :key="row">
				<div class="tvButton" v-for="button in 3" :key="button" @click="sendSignal((row * button) + 1)">
					{{ row * button }}
				</div>
			</div>
			<div class="tvButtonRow">
				...
				<div class="tvButton" @click="sendSignal(0)">
					0
				</div>
				<div class="tvButton">
					EXIT
		...
		<div class="channels" v-if="selectedTab=='channels'">
			<div class="matchLive" v-if="lechMatches().length">
				...
				<div>
					<tvChannel v-for="(channel, index) in lechMatches()"  
						:channelData="channel" 
						:recieverIp="recieverIp"
						:key="index" 
						@sendChannelChangeSignal="sendChannelChangeSignal" 
						class="gray"></tvChannel>
				</div>
			</div>
			<div class="categories">
				<div class="item" v-for="filter in filters" :key="filter" :class="selectedFilter==filter.value ? 'selected' : ''" @click="changeFilter(filter.value)">
					{{ filter.name }}
				</div>
			</div>
			<tvChannel v-for="(channel, index) in channels.filter(obj => JSON.stringify(obj).includes(selectedFilter || ''))" 
				:channelData="channel" 
				:recieverIp="recieverIp"
				:key="index" 
				@sendChannelChangeSignal="sendChannelChangeSignal"></tvChannel>
...
export default {
  ...
  data() {
    return {
		...
		history:[],
		recieverStatus: {"inStandby": "false", "currservice_begin": "05:10", "muted": true, "isRecording": "false", "volume": 100, "currservice_fulldescription": "N/A", "currservice_name": "N/A", "currservice_filename": "", "transcoding": false, "currservice_end": "10:20", "currservice_description": ""}
    };
	},
	created: function () {
		const vm = this;
		vm.recieverIp = vm.deviceAddress.ip;
		for(let channelIndex = 0 ; channelIndex < vm.channels.length ; channelIndex++){
	...
	methods: {
		getStatus: function(signal){
			...
			try{
				...
				request.open('GET', "http://"+vm.recieverIp+"/api/statusinfo", true);

				request.onreadystatechange = function(event) {
					if (this.readyState === XMLHttpRequest.DONE) {
						if (this.status === 200) {
							vm.recieverStatus = JSON.parse(request.response);
							alert(vm.recieverStatus);
							console.log(vm.recieverStatus.currservice_serviceref);
							if(vm.recieverStatus.currservice_name == "N/A"){
								setTimeout(function(){
									vm.getStatus();
								},2000)
							}
		...
		sendSignal: function(signal){
			const vm = this;
			$.ajax({
				url: "http://"+vm.recieverIp+"/api/remotecontrol",
				headers: {  
		...
		sendChannelChangeSignal: function(signal){
			const vm = this;
			$.ajax({
				url: "http://"+vm.recieverIp+"/api/zap",
				headers: {  
					"content-type": "application/x-www-form-urlencoded"  
				},
				data: {
		...
		sendPowerSignal: function(signal){
			const vm = this;
			$.ajax({
				url: "http://"+vm.recieverIp+"/api/powerstate",
				data: {
				newstate: signal
				},
				dataType: "text"
		...
		refreshEPG: function(){
			...
			for(let channelIndex = 0; channelIndex < vm.channels.length ; channelIndex++){
				let request = new XMLHttpRequest();
				request.open('GET', "http://"+vm.recieverIp+"/api/epgservicelistnownext?sList="+vm.channels[channelIndex].id, true);

				request.onreadystatechange = function(event) {
					if (this.readyState === XMLHttpRequest.DONE) {
						if (this.status === 200) {
							vm.channels[channelIndex].epg = JSON.parse(request.response);
							// vm.recieverStatus.currservice_name = "xD";
							// console.info('Cross origin request was sent with success!', this.responseText);
						} else {
							// console.error('Cross origin request failed :(', this.status, this.statusText);
						}
		...
		changeFilter: function(filter){
			const vm = this;
			vm.selectedFilter = vm.selectedFilter == filter ? undefined : filter;
		},
		lechMatches: function(){
			const vm = this;
			return vm.channels.filter(obj => obj.epg.events.length > 1 && obj.epg.events[0].title && obj.epg.events[1].title && (obj.epg.events[0].title.toUpperCase().includes('LECH P') || obj.epg.events[1].title.toUpperCase().includes('LECH P')));
		}
	},
	computed:{
		thisDeviceData(){
	...
	watch: {
		recieverStatus: {
			...
			handler(){
				...
				let historyIndex = vm.history.findIndex(obj=> obj.name == vm.recieverStatus.currservice_station);
				if(historyIndex == -1 && vm.recieverStatus.currservice_name != "N/A"){
					vm.history.push({
