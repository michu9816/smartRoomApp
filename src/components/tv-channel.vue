<template>
	<div class="channel" :class="channelClass">
		...
		<div class="changeChannelDiv" @click="changeChannel(channelData.id)"></div>
		<div class="loadingInfo" :class="{swipeLeft: !loadingInfo}">
			<img :src="channelIconHref">
			<p>Wczytywanie programu TV ...</p>
			<p style="font-weight:bold;font-size:20px"> {{ channelData.name }} </p>
		</div>
		<div class="channelName">
			{{ channelData.name }}
		</div>
		<div class="current" :style="'background:-webkit-linear-gradient(left,rgb(0 95 138 / 40%) '+getPercentage()+'%,transparent 0%)'">
			{{ channelData.epg.events[0].title }}
		</div>
		<div @click="playStream(channelData.id)" style="padding:5px 10px;font-size:12px;" class="next">
...
export default {
	...
	methods:{
		...
		playingLechMatch: function(id){
			...
			return (obj.epg.events.length > 1 
		...
		getPercentage: function(id){
			...
			const endTime = parseInt(event.begin_timestamp) + parseInt(event.duration_sec);
			const now = new Date();
			const currentProgress = now.getTime() - (startTime * 1000);

			return parseInt(currentProgress / (parseInt(event.duration_sec) * 1000) * 100)
		}
	},
	computed:{
		channelClass(){
		...
		startTime(){
			...
			return channelData.epg 
				? this.getHumanTime(channelData.epg.events[1].begin_timestamp * 1000) 
				: "--:--";
		},
		endTime(){
			const channelData = this.channelData;
			return channelData.epg 
				? this.getHumanTime(channelData.epg.events[1].begin_timestamp * 1000 + (channelData.epg.events[1].duration_sec * 1000)) 
				: "--:--";
		},
		loadingInfo(){
			const vm = this;
